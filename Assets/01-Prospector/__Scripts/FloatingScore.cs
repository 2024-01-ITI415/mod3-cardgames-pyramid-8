...
public class FloatingScore : MonoBehaviour
{
    ...
    // The score property sets both_score and scoreString
    public int score {
        get {
            return(_score);
        }
    ...
    void Update() {
        ...
        if(u < 0) { // u < 0, then we shouldn't move yet
            state = eFSState.pre;
            ...
            if(u >= 1) { // If u >= 1, we're done moving
                ...
                else { // If there is nothing callback
                    // ... then don't destroy this. Just let it stay still.
                    state = eFSState.idle;
                }
            }
            else { // 0 <= u < 1, which means that this active and moving
            ...
            // RectTransform anchors can be used to position UI objects relative to
            // total size of the scree
